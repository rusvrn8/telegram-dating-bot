...
try:
    ...
    if users and len(users) > 0:
        ...
        filters = filter_users('gender', 'Парень', users, test_user_id)
        print(f"  ✅ Фильтрация работает. Найдено: {len(filters)}")
    else:
        print("  ⚠️  Нет пользователей для фильтрации")
except Exception as e:
...
print("или")
print("  .\\venv\\Scripts\\python.exe my_bot_new.py")
