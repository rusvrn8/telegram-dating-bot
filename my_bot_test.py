...
def filter(fil_key, fil_val, users_dict, id):
    if fil_val and isinstance(fil_val, str):
        filter_users = {u for u in users_dict if
                        u != id and u not in users_dict[id]['seen'] and users_dict[u][fil_key] == fil_val}
    elif fil_val and isinstance(fil_val, list):
        filter_users = {u for u in users_dict if
                        u != id and u not in users_dict[id]['seen'] and users_dict[u][fil_key] in fil_val}
    else:
...
async def send_question(update, context):
    ...
    await update.message.reply_text(f"Привет! 😄 Прежде чем перейти к регистрации, мне необходимо убедиться"
    ...
    await update.message.reply_text("Выбери правильный ответ.", reply_markup=ReplyKeyboardMarkup([[answer] for answer in answers],
    ...
    if state == 'name':
        ...
        if query == "Пол":
            ...
            await update.message.reply_text("Выбери цель знакомства  🎯.",
                                            reply_markup=ReplyKeyboardMarkup([[keyboard] for keyboard in keyboards],
            ...
            keyboard = ReplyKeyboardMarkup([[region] for region in regions], one_time_keyboard=True,
                                           resize_keyboard=True)
        ...
        await update.message.reply_text(
            ...
            reply_markup=ReplyKeyboardMarkup([[keyboard] for keyboard in keyboards], one_time_keyboard=True,
                                             resize_keyboard=True))
    elif state == 'goal':
...
async def search(update, context):
    ...
    if unseen_users:
        ...
        await update.callback_query.message.reply_text('Анкеты закончились, приходи позже...',
                                                       reply_markup=reply_markup)
...
async def check_likes(context):
    ...
    user_id = str(user_id)
    logging.info(f'{user_id = }')
    likes_users = [u for u in likes if
                   u != user_id and u not in likes[user_id]['seen'] and u in likes[user_id]['likes']]
...
if __name__ == '__main__':
    ...
    application.add_handler(MessageHandler(
        (filters.TEXT & (~filters.COMMAND)) | filters.Regex("^(Парень|Девушка|Общение|Дружба|Отношения|Интим)$"),
        input_data))
